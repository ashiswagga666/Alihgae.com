...
<section class="bg-gradient-to-br from-blue-700 to-blue-900 py-16 text-white">
    <div class="max-w-4xl mx-auto px-6 text-center">
        <h1 class="text-4xl font-bold mb-3">📰 Berita & Artikel</h1>
        <p class="text-blue-200 text-lg">Tips karir, berita kerja, dan informasi terbaru seputar dunia kerja Bali</p>
        <form method="GET" action="{{ route('berita.index') }}" class="mt-6 max-w-md mx-auto flex gap-2">
            ...
            <button class="bg-white text-blue-700 px-6 py-3 rounded-xl font-semibold text-sm hover:bg-blue-50 transition">Cari</button>
        </form>
    </div>
</section>
    ...
    <div class="max-w-6xl mx-auto px-6">
        ...
        <div class="flex gap-2 flex-wrap mb-8">
            @foreach(['' => 'Semua', 'berita' => 'Berita', 'tips-karir' => 'Tips Karir', 'berita-perusahaan' => 'Berita Perusahaan'] as $k => $label)
                <a href="{{ route('berita.index', ['kategori' => $k]) }}" class="px-4 py-2 rounded-full text-sm font-medium transition {{ request('kategori') === $k ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-blue-50 border border-gray-200' }}">{{ $label }}</a>
            @endforeach
        </div>
        ...
        <a href="{{ route('berita.show', $featured->slug) }}" class="block mb-8 bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-xl transition-all group">
            <div class="grid grid-cols-1 md:grid-cols-2">
                <div class="h-56 md:h-auto bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center overflow-hidden">
                ...
                <div class="p-8 flex flex-col justify-center">
                    <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-full w-fit mb-3">{{ ucfirst(str_replace('-', ' ', $featured->kategori)) }}</span>
                    <h2 class="text-2xl font-bold text-gray-800 group-hover:text-blue-700 transition mb-3">{{ $featured->judul }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <a href="{{ route('berita.show', $b->slug) }}" class="bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden">
                <div class="h-44 bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center overflow-hidden">
                    @if($b->thumbnail)
                ...
                <div class="p-5">
                    <div class="flex items-center gap-2 mb-2">
                        <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ ucfirst(str_replace('-', ' ', $b->kategori)) }}</span>
                        @if($b->is_sponsored)<span class="text-xs text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full">Sponsor</span>@endif
                    </div>
                    <h3 class="font-bold text-gray-800 text-sm leading-snug group-hover:text-blue-700 transition mb-2">{{ $b->judul }}</h3>
