...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <div class="text-center mb-4">
            @if($profile->photo)
                <img src="{{ asset('storage/'.$profile->photo) }}" class="w-20 h-20 rounded-full mx-auto object-cover border-4 border-green-100">
            @else
                <div class="w-20 h-20 rounded-full mx-auto bg-green-100 flex items-center justify-center text-3xl font-bold text-green-600">
        ...
        <div class="space-y-2 text-sm">
            <div class="flex items-center gap-2 text-gray-600">
                <i class="fas fa-graduation-cap w-4 text-green-500"></i>
        ...
        <a href="{{ route('pelamar.profil') }}" class="mt-4 block w-full text-center bg-green-600 text-white py-2 rounded-xl text-sm font-semibold hover:bg-green-700 transition">
    ...
    <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('pelamar.riwayat') }}" class="text-sm text-green-600 hover:underline">Lihat Semua</a>
        </div>
        ...
        <div class="text-center py-8 text-gray-400">
            ...
            <a href="{{ route('lowongan') }}" class="mt-2 inline-block text-green-600 text-sm hover:underline">Cari Lowongan Sekarang</a>
        </div>
        @endforelse
    </div>
</div>
...
<div class="mt-6 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('lowongan') }}" class="text-sm text-green-600 hover:underline">Lihat Semua</a>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @foreach($lowonganRekomendasi as $job)
        <a href="{{ route('lowongan.detail', $job->id) }}" class="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-green-200 hover:bg-green-50 transition">
