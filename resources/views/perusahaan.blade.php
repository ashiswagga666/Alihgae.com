...
<section class="bg-gradient-to-br from-green-700 via-green-800 to-green-900 py-14 text-white">
    <div class="max-w-5xl mx-auto px-6 text-center">
        ...
        <p class="text-green-200">{{ $perusahaans->count() }}+ perusahaan aktif merekrut talenta terbaik</p>
    </div>
</section>
...
<div class="max-w-6xl mx-auto px-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <a href="{{ route('perusahaan.detail', $p->id) }}" class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-xl hover:-translate-y-1 transition-all border border-gray-100 group">
            <div class="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center text-3xl mb-4 overflow-hidden">
                @if($p->logo)
                    <img src="{{ asset('storage/'.$p->logo) }}" class="w-full h-full object-cover">
                @else 🏢 @endif
            </div>
            <h2 class="font-bold text-lg text-gray-800 group-hover:text-green-700 transition">{{ $p->company_name }}</h2>
            ...
            <div class="flex items-center justify-between mt-4">
                @if($p->is_verified)
                    <span class="text-xs text-green-600 bg-green-50 px-2 py-1 rounded-full"><i class="fas fa-check-circle mr-1"></i>Terverifikasi</span>
